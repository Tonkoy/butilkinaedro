...
export function ProductListView() {
  ...
  const handleViewRow = useCallback(
    (slug) => {
      router.push(paths.dashboard.product.details(slug));
    },
    [router]
  );
  ...
  return (
    <RoleBasedGuard hasContent currentRole={user?.role} acceptRoles={['admin']} sx={{ py: 10 }}>
      <DashboardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: tCommon('breadcrumbs.products'), href: paths.dashboard.product.root },
            { name: tCommon('breadcrumbs.dashboard') },
          ]}
          action={
            <Button
      ...
      <ConfirmDialog
        ...
        content={
          <>
            Сигурен ли си, че искаш да изтриеш <strong> {selectedRowIds.length} </strong> продукта?
          </>
        }
        action={
          <Button
      ...
      <GridToolbarContainer>
        ...
        >
          {!!selectedRowIds.length && (
            ...
            >
              {tCommon('buttons.delete')} ({selectedRowIds.length})
            </Button>
          )}
