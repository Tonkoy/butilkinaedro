...
function Container({ children }) {
  ...
  const completed = activeStep === PRODUCT_CHECKOUT_STEPS.length - 1;
    ...
    (newItem) => {
      const updatedItems = state.items.map((item) => {
        if (item.id === newItem.id) {
          ...
          return { ...item, colors, quantity: item.quantity + newItem.quantity };
        }
        return item;
      });
  ...
  const onCreateTracking = useCallback(
    (trackingInfo) => {
      setField('tracking', trackingInfo);
...
function createUrl(type, activeStep) {
  ...
  return `${paths.products.checkout}?${stepParams}`;
}
